      ...
      <div className="flex flex-col flex-1 ml-sidebar-width">
        ...
        <main className="pt-16 min-h-screen p-gutter-lg flex flex-col gap-stack-md">
          <div className="mt-8">
            ...
            <p className="font-body-md text-on-surface-variant">ERA5 Reanaliz modelinə əsaslanan 10 illik iqlim analizi</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-gutter-md">
            ...
            <StatCard 
              ...
              value={archiveData?.daily?.wind_speed_10m_max[0] || "58"} 
              unit="km/h" 
              icon="Wind" 
              loading={loading}
              description="ERA5 modeli ilə ölçülmüş maksimal külək sürəti."
            />
          </div>

          <div className="w-full bg-surface-container-lowest border border-outline-variant/30 rounded-xl p-6 shadow-sm">
            <div className="flex justify-between items-start mb-6">
              <div>
                <h3 className="font-headline-md">Temperatur Tarixçəsi və Proyeksiyası</h3>
                <p className="text-xs text-on-surface-variant mt-1">Bütöv xətt: ERA5 Tarixi Məlumat, Qırıq xətt: 10 illik Lineer Proqnoz</p>
              </div>
              <div className="flex gap-2">
                <span className="text-[10px] bg-red-500/10 text-red-500 px-2 py-1 rounded-full font-bold uppercase">ERA5 Model</span>
                <span className="text-[10px] bg-primary/10 text-primary px-2 py-1 rounded-full font-bold uppercase">Proqnoz</span>
              </div>
            </div>
            ...
            <div className="mt-6 p-4 bg-surface-container rounded-lg border border-outline-variant/20">
              <p className="text-sm text-on-surface-variant leading-relaxed">
                <strong className="text-on-surface">Analitik İzah:</strong> Bu qrafik Open-Meteo Historical API vasitəsilə əldə edilən <strong className="text-on-surface">ERA5</strong> reanaliz verilənlərini əks etdirir. ERA5 ECMWF tərəfindən hazırlanmış qlobal iqlim modelidir. Proyeksiya <code className="bg-black/20 px-1 rounded">T = T₀ + 0.032Δt</code> tənliyi əsasında qurulmuşdur. Bu model Xəzər hövzəsi üçün xarakterik olan illik istiləşmə trendini simulyasiya edir və növbəti onillik üçün ekoloji riskləri qiymətləndirməyə kömək edir.
              </p>
            </div>
          </div>
